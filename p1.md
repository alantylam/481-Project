...
According to our user research, fabrication labs are sometimes used as a place for courses, but they are mainly for research uses. And the individual work time is up to 6 hours. Through our interview, observation, and secondary research, we found that the number of machines in the lab is limited. Therefore, it is inconvenient for users to use the machines at any time they want. They usually do not know the status of the machine they want to use unless they go to the lab to check by themselves. For example, if one of the users took 30 mins come to the lab trying to do research and found that all the machines are occupied, he/she would waste a lot of time. In addition, if the machine they need has been used by someone, they have to come to ask that person to clean the machine. Since it is quite inconvenient for users, they would like to know the status of machines and when people use it.
